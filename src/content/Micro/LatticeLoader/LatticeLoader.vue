...
const HOLE_CLASS = `${CELL} [opacity:calc(var(--ll-idle)*0.47)]`;
...
const LIT_CLASS =
  '[opacity:var(--ll-idle)] [animation-timing-function:var(--ll-ease-in-out)] group-data-[glow]:[box-shadow:0_0_calc(var(--ll-cell)*1.2)_calc(var(--ll-cell)*0.12)_var(--ll-glow)]';
const MARK_CELL_CLASS =
  '[opacity:var(--ll-idle)] [transition:opacity_200ms_ease,background-color_200ms_ease] data-[on]:[background:var(--ll-mark)] data-[on]:[opacity:var(--ll-peak)] group-data-[glow]:data-[on]:[box-shadow:0_0_calc(var(--ll-cell)*1.2)_calc(var(--ll-cell)*0.12)_var(--ll-mark-glow)]';
const TEXT_CLASS =
  'll-text absolute top-0 left-0 whitespace-nowrap opacity-0 [filter:blur(2px)] [transition:opacity_200ms_ease,filter_200ms_ease] data-[active]:static data-[active]:opacity-100 data-[active]:[filter:blur(0)]';
...
const props = withDefaults(defineProps<LatticeLoaderProps>(), {
  label: 'Thinking',
  doneLabel: 'Done in',
...
<template>
  <span
    role="status"
    class="ll-root group relative inline-flex items-center leading-none [font-family:inherit] [gap:calc(var(--ll-font)*0.625)] [font-size:var(--ll-font)]"
    :class="className"
    :data-status="status"
    :data-shape="shape"
  ...
  >
    ...
    <span
      v-if="showTimer"
      ref="timerRef"
      class="opacity-60 font-mono tabular-nums [font-size:calc(var(--ll-font)*0.875)]"
      aria-hidden="true"
    >
      0.0s
    </span>
