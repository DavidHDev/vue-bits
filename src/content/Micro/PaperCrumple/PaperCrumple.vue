...
const props = withDefaults(defineProps<PaperCrumpleProps>(), {
...
const emit = defineEmits<{ stateChange: [state: PaperCrumpleState]; error: [error: Error] }>();

const pathCache: { current: { key: string; value: ReturnType<typeof createPaperPath> } | null } = { current: null };
...
// the live options are read through a getter, like the React ref, so the render loop always sees fresh props
const options = {
  get current() {
    return {
      releaseBehavior: props.releaseBehavior,
      crumpleAmount: props.crumpleAmount,
      crumpleDuration: props.crumpleDuration,
      releaseDuration: props.releaseDuration,
      creaseStrength: props.creaseStrength,
      draggable: props.draggable,
      dragRotation: props.dragRotation,
      dragRadius: props.dragRadius,
      returnToOrigin: props.returnToOrigin,
      disabled: props.disabled,
      onStateChange: (next: PaperCrumpleState) => emit('stateChange', next),
      onError: (error: Error) => emit('error', error)
    };
  }
};

// the whole three.js scene is built here and rebuilt whenever a geometry or material prop changes
const mountPaper = (): (() => void) | undefined => {
  const {
    src,
    backSrc,
    width,
  ...
  function finish(instant = false) {
    ...
    keyboard = false;
    publish(
      opts.releaseBehavior === 'stay' && amount.target > 0.001 ? 'crumpled' : memory.target > 0.001 ? 'creased' : 'flat'
    );
    wake();
  }
...
// runs once the refs exist (an immediate watcher would run before mount)
...
const shadowFilter = () =>
  props.shadow ? `drop-shadow(0 6px 10px rgb(0 0 0 / ${clamp(finite(props.shadowOpacity, 0.08), 0, 1)}))` : undefined;
</script>

<template>
  <div
    ref="rootRef"
    class="isolate relative w-full overflow-hidden"
    :class="className"
  ...
  >
    ...
    <img
      v-if="status !== 'ready'"
      class="top-1/2 left-1/2 absolute w-[var(--pc-image-width,60%)] h-[var(--pc-image-height,72%)] pointer-events-none select-none"
      :src="src || undefined"
      :alt="alt"
      draggable="false"
      :style="{
    ...
    <button
      ref="hitRef"
      type="button"
      class="peer top-0 left-0 absolute bg-transparent shadow-none! m-0 p-0 border-0 rounded-[2px] outline-none! focus-visible:outline-none! focus:outline-none! appearance-none touch-none cursor-grab data-[held=true]:cursor-grabbing disabled:cursor-default select-none [-webkit-tap-highlight-color:transparent]"
    ...
    />
    <span
      class="bottom-6 left-[calc(50%-2px)] absolute bg-current opacity-0 peer-data-[pointer=true]:opacity-0! peer-focus-visible:opacity-55 rounded-full w-1 h-1 pointer-events-none"
      aria-hidden="true"
    />
