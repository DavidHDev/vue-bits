...
type Settings = Required<Omit<TechTextProps, 'className'>>;
...
const props = withDefaults(defineProps<TechTextProps>(), {
  text: 'Vue Bits',
  fontFamily: '',
  fontWeight: 600,
  fontSize: 150,
...
const mountScene = () => {
  ...
  if (!container || !canvas || !ctx || !scratchCtx) return;
  ...
  const setFont = (target: CanvasRenderingContext2D, s: Settings, size: number) => {
    ...
    if ('letterSpacing' in target) (target as CanvasRenderingContext2D).letterSpacing = `${s.letterSpacing * size}px`;
...
<template>
  <div
    ref="containerRef"
    class="relative w-full h-full touch-pan-y select-none"
    :class="className"
    role="img"
    :aria-label="text"
  >
