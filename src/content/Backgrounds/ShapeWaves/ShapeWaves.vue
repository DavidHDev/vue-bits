...
const props = withDefaults(defineProps<ShapeWavesProps>(), {
  text: '',
...
const settings = computed<ShapeWavesSettings>(() => ({
  text: String(props.text ?? ''),
  fontFamily: props.fontFamily,
  fontWeight: props.fontWeight,
  textSize: props.textSize,
  shapes: props.shapes,
  cellSize: Math.max(2, props.cellSize),
  dotSize: props.dotSize,
  color: props.color,
  hoverColor: props.hoverColor,
  backgroundColor: props.backgroundColor,
  speed: props.speed,
  scale: Math.max(0.05, props.scale),
  ...
  splashStrength: props.splashStrength,
  glow: props.glow,
  intro: props.intro,
  introDuration: Math.max(0.1, props.introDuration),
...
const maskSignature = computed(() => [props.text, props.fontFamily, props.fontWeight, props.textSize].join('|'));
...
watch(settingsSignature, () => applySettings());
watch(maskSignature, () => applyMask());
watch(
  () => props.introKey,
  () => {
    if (props.introKey) replay();
  }
);
...
onMounted(() => {
  ...
  const splash = (x: number, y: number, strength: number) => {
    ...
    const sigma = Math.max(0.5, ((current.splashRadius * dpr) / cellPx) * 0.5);
  ...
  const handlePointerMove = (event: PointerEvent) => {
    if (!settings.value.interactive) return;
    ...
    if (inside) {
      ...
      splash(x, y, Math.min(1, 0.22 + speed * 0.0006) * settings.value.splashStrength);
      wakeRenderer();
    }
    pointer.x = x;
    pointer.y = y;
  ...
  void (async () => {
    try {
      ...
      const glowEnabled = () => settings.value.glow > 0;
      let lastIntro = false;

      const configureGrid = () => {
        const current = settings.value;
        const [width, height] = output.size;
        const nextCols = Math.max(1, Math.round(width / (current.cellSize * dpr)));
      ...
      const isAnimating = () => {
        const current = settings.value;
        return visible && !document.hidden && !current.paused && current.speed > 0 && !reduceMotion.matches;
      };
      ...
      const render = (now: number) => {
        ...
        if (animating) {
          time += deltaSeconds * TIME_RATE * current.speed;
          const angle = (current.direction * Math.PI) / 180;
          const distance = current.flow * cellPx * deltaSeconds;
        ...
        if (introPlaying) {
          introProgress = Math.min(INTRO_END, ((now - introStart) / 1000 / current.introDuration) * INTRO_END);
        }
        params.set({ placement: [gridOrigin[0], gridOrigin[1], rows, introProgress] });
        params.set({
          field: [
            NOISE_CELLS * cellPx * current.scale,
            0.5 - (current.brightness - 0.5) * 0.4,
            2.8 * current.contrast,
            time
          ]
        });
        params.set({ motion: [drift[0], drift[1], current.text.trim() ? 1 : 0, current.fade] });
        try {
      ...
      const drawMask = () => {
        ...
        if (hasText) {
          let fontPx = Math.max(1, current.textSize * maskHeight);
          maskContext.font = `${current.fontWeight} ${fontPx}px ${current.fontFamily}`;
      ...
      const runApplySettings = () => {
        if (disposed || failed) return;
        const current = settings.value;
        configureGrid();
        params.set({
          placement: [gridOrigin[0], gridOrigin[1], rows, introProgress],
          grid: [cellPx, Math.min(1, Math.max(0.1, current.dotSize)), SHAPE_MODES[current.shapes] ?? 0, cols],
          color: [...parseColor(current.color, '#929292'), 1],
          hover: [...parseColor(current.hoverColor, '#ffffff'), glowEnabled() ? 1 : 0],
          background: [...parseColor(current.backgroundColor, '#000000'), 1]
        });
        compositeParams.set({ strength: [2 * current.glow, 0, 0, 0] });
        if (current.intro !== lastIntro) {
          lastIntro = current.intro;
          if (current.intro && !reduceMotion.matches) introArmed = true;
        }
        if (!current.interactive && chargesActive) {
          heights.fill(0);
          previousHeights.fill(0);
          charges.fill(0);
          chargesActive = false;
      ...
      const runApplyMask = () => {
        ...
        runApplySettings();
        const current = settings.value;
        if (!current.text.trim() || !document.fonts?.load) return;
        document.fonts
          .load(`${current.fontWeight} 32px ${current.fontFamily}`)
          .then(() => {
            if (disposed || failed) return;
            drawMask();
            wakeRenderer();
      ...
      applySettings = runApplySettings;
      applyMask = runApplyMask;
      replay = () => {
        if (disposed || failed || !settings.value.intro || reduceMotion.matches) return;
        introArmed = true;
        wakeRenderer();
      };
...
<template>
  ...
  >
    <canvas
      ref="canvasRef"
      class="block absolute inset-0 w-full h-full transition-opacity motion-reduce:transition-none duration-[600ms] ease-[cubic-bezier(0.22,1,0.36,1)] pointer-events-none"
